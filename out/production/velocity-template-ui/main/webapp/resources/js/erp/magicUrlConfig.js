...
function fillChannelDrop(selectbody, data){
    if(data.length == 0){
    	notifyWarn({"title": "Warning", "value": "Please Configure ERP Channels first"});
        return;
    }
  var $selectbody = selectbody;
  $selectbody.empty();
  var row = "<option selected hidden disabled>Select ERP Channel</option>"
...
function searchUrls() {
    ...
    if(channelAlias == "" || channelAlias == "Select ERP Channel"){
...
function createUrlTable(data) {
  ...
  var buttonHtml = ' <button class="btn-sm btn btn-outline-primary" onclick="addUrl(\'' + data.channelAlias + '\',\'' + e.urlConstants + '\',\'' + e.url + '\', \'update\')">Update</button>';
  var row = '<tr>'
...
function submit() {
    ...
    if(channelAlias == "" || channelAlias == "Select ERP Channel" || constant == "" || constant == "Select URL Name" || url == "") {
...
function getConstants(){
    var url = getMagicUrlConfigUrl() + "/constants/erp";
